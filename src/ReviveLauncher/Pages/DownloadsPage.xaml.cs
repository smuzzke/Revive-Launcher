            ...
            try
            {
                if (DownloadBuild.Content == "Cancel")
                {
                    ...
                    if (File.Exists(_downloadPathFull))
                    {
                        ...
                        {
                            DialogService.ShowSimpleDialog("Failed to download 8.51 build: " + ex.Message, "Error");
                        }
                    }
                ...
                if (folder != null)
                {
                    ...
                    if (availableSpace >= requiredSpace)
                    {
                        ...
                        _downloadPathFull = Path.Combine(path, "8.51.rar");
                        _downloadPath = path;
                        _webClient.DownloadProgressChanged += Wc_DownloadProgressChanged;
                        _webClient.DownloadFileCompleted += Wc_DownloadFileCompleted;
                        _webClient.DownloadFileAsync(new Uri("https://cdn.fnbuilds.services/8.51.rar"), _downloadPathFull);
