...
namespace ReviveLauncher.Pages
{
    ...
    public sealed partial class SettingsPage : Page
    {
        ...
        private async void CheckForUpdates_Click(object sender, RoutedEventArgs e)
        {
            if (Definitions.outdated == true && Definitions.limitguestfeatures == true)
            {
                DialogService.ShowSimpleDialog("Please update Revive Launcher to use this feature.", "Error");
            }
            else
            {
                ...
                if (Definitions.APIversion != null)
                {
                    ...
                    {
                        DialogService.ShowSimpleDialog("New update detected current version: " + Definitions.CurrentVersion + " Latest version: " + Definitions.APIversion + " please update to continue using revive launcher.", "New update");
                        Environment.Exit(1);
                    }
                    else
                    {
        ...
        private async void RemovePatch_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                string reviveEACPath = System.IO.Path.Combine(FNPath, "ReviveClient-Win64-Shipping.exe");
                if (File.Exists(reviveEACPath))
                {
                    File.Delete(reviveEACPath);
                }
