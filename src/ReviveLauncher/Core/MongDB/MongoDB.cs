...
using ReviveLauncher.Core.Loops;
using Microsoft.UI.Xaml;
using CommunityToolkit.WinUI.Helpers;

namespace ReviveLauncher.MongoDB
{
    public static class MongoDBAuthenticator 
    {
        public static async Task<string> CheckLogin(string email, string password)
        {
            try
            {
                string cng = await API.MongoDBConntectionString();
                ...
                // Access the database
                var database = client.GetDatabase("ReviveBackend");
                ...
                if (user != null)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (user.RevivePass)
                            {
                                MainShellPage.DisplayUsername = user.username.ToString() + "[Revive Pass]";
                            }
